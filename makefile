...
COMPILE_FLAGS = -MMD -g -O3 -mips16 -mprocessor=$(CPU) -c
COMPILE_CFLAGS = $(COMPILE_FLAGS) -x c -Wall
COMPILE_CPPFLAGS = $(COMPILE_FLAGS) -x c++ -Wall

LD_FLAGS = -O3 -mprocessor=$(CPU) -nostartfiles  -Wl,--defsym,_min_heap_size=0,-Map=$(BINDIR)/$(PROJECT).map,--gc-sections,--report-mem
